...
import type { BuilderField, BuilderSection, SectionDirection } from '../types'

export function useFormSchema(
  fields: Ref<BuilderField[]>,
  sections: Ref<BuilderSection[]>,
  sectionDirection: Ref<SectionDirection>,
  onSubmitCallback?: (schema: FormSchema) => void | Promise<void>,
) {
  const isSubmitting = ref(false)
  const { saveFormSchema } = useFormStore()

  function mapField(field: BuilderField) {
    return {
      id: field.id,
  ...
  function exportSchema(): FormSchema {
    return {
      fields: fields.value.map((field) => mapField(field)),
      sectionDirection: sectionDirection.value,
